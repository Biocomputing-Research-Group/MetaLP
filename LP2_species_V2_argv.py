...
import sys
import getopt
from protein2species import pro2spec
from protein2species import pro2specSoil
...
def write_solution(model, peptideID, proteinID, pep_prob_dict, peptide2protein_dict, protein2peptide_dict,
                         proteinGroup2peptide, proteinGroupIDs, peptideIDPro_dict, proteinGroup_dict, VarIDX, VarIDX2):
    start=time.time()
    pro_prob_dict = {}
...
def speciePro(filename, proteinID):
    ...
    with open(filename) as f:
        for line_id, line in enumerate(f):
            ...
            #if float(prob)>0.000567908:
            #    prob_dict[specie]=1
            #else:
            #    prob_dict[specie] = float(prob)

    proID2species_dict = pro2spec()
    #probability_base=max(prob_dict.values())
...
def specieProUnknown(filename, proteinID):
    proteinIDPro_dict = dict()
    prob_dict = dict()
    with open(filename) as f:
        for line_id, line in enumerate(f):
...
def proteinFDR(filename, fdr_float,filteringfile):
    filtering_proteins = []
    data = []
    decoy = 0
    target = 0
    ...
    print('number of target proteins within FDR ' + str(fdr_float) + ': ' + str(len(proteins)))
...
if __name__ == "__main__":
    ...
    argv = sys.argv[1:]
    try:
        opts, args = getopt.getopt(argv, "hi:f:o:d:r:p:l")
    except:
    ...
    if len(opts) == 0:
        ...
        print("-d\t Protein database, fasta format\n")
        print("-r\t The OTU mapping information file for metagenomics if the species are unknown, this file contains the mapping between assembly contigs and the indices of each OTU. If the species for the metaproteome is known, please ignore this option eg. For Mock community, the species are already known.")
        print("-p\t The probability of species\n")
        print("-l\t Filtering result with specific FDR in protein level\n")
        sys.exit(1)
    inputfile = ""
    outputfile = ""
    fastafile= ""
    ...
    if refFlag==0:
        proteinIDPro_dict = speciePro(probabilityfile, fastafile, proteinID)
    else:
        proteinIDPro_dict = specieProUnknown(probabilityfile, fastafile, refFlag,proteinID)
    ...
    proteinFDR(outputfile, fdr,filteringfile)
    end = time.time()
